...
def map_font_size_to_levels(font_sizes):
    ...
    for size, _ in sorted_by_size:
        if size in (body_font, title_font):
            continue
        # Promote the largest size to H1 if it looks like a major section title
        if heading_level == 1:
            mapping[size] = 'H1'
            heading_level += 1
            continue
...
def is_heading_candidate(text, is_bold, font_size, median_size):
    ...
    # Relaxed thresholds to allow longer headings
    if word_count > 15:
        return False
    if char_count / max(word_count, 1) > 20:
        return False
    if '\n' in text:
        return False
    
    # Accept if bold or font size significantly larger than median
    if is_bold or font_size > median_size * 1.2:
        return True
...
def detect_headings(blocks, title=None, excluded_title_text=None, max_level=3, debug=False):
    ...
    for size in blocks_by_size:
        ...
        for i, b in enumerate(blocks_by_size[size]):
            ...
            # New heuristic: if text is all caps or bold and followed by next line, consider heading
            next_line = blocks_by_size[size][i+1] if i+1 < len(blocks_by_size[size]) else None
            if (text.isupper() or is_bold) and next_line:
                if debug:
                    print(f"Heuristic: '{text}' is all caps or bold and followed by next line, considering as heading")
                candidates.append((text, level_idx, b['page'], b['bbox'], is_bold, b['size']))
                continue
    ...
    unique_outline.sort(key=lambda h: (
        h['page'],
        # Removed bbox sorting since bbox is no longer included
